        ...
        <div className="ml-auto flex items-center gap-2">
          {onToggleVista && (
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-emerald-900/10 bg-white/70 px-3 py-1.5 text-[12.5px] font-medium text-slate-600 transition hover:border-emerald-500/40"
              title={esDesktop ? 'Vista móvil' : 'Vista escritorio'}
            >
              {esDesktop ? <Smartphone className="size-4" /> : <Monitor className="size-4" />}
              <span className="hidden sm:inline">{esDesktop ? 'Vista móvil' : 'Vista escritorio'}</span>
            </button>
          )}
          <div className="flex shrink-0 items-center gap-2 rounded-full bg-emerald-500/[0.08] py-1 pl-1 pr-3">
            <span className="grid size-7 place-items-center rounded-full text-[11px] font-bold text-white" style={{ background: 'linear-gradient(135deg,var(--p6),var(--a6))' }}>{iniciales}</span>
